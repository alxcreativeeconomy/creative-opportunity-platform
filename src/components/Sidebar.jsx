...
import { TrendingUp, Search, FileSearch, Menu, Shield, X } from 'lucide-react';
    ...
    <>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-50 ${isSidebarOpen ? 'w-72 translate-x-0' : 'w-20 lg:translate-x-0 -translate-x-full'} bg-white border-r border-slate-200 transition-all duration-300 flex flex-col shadow-xl lg:shadow-none`}>
        ...
        <nav className="flex-1 px-4 space-y-2 mt-6">
          <SidebarItem id="dashboard"    icon={TrendingUp}  label="Overview"       activeTab={activeTab} onClick={handleNav} isSidebarOpen={isSidebarOpen} />
          <SidebarItem id="search"       icon={Search}      label="Matrix"         activeTab={activeTab} onClick={handleNav} isSidebarOpen={isSidebarOpen} />
          <SidebarItem id="deals-report" icon={FileSearch}  label="Deals Report"   activeTab={activeTab} onClick={handleNav} isSidebarOpen={isSidebarOpen} />
        </nav>
